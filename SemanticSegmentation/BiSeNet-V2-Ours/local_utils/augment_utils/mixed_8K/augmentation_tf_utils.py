...
if __name__ == '__main__':
    ...
    test code
    """
    source_image_path = '/media/baidu/DataRepo/IMAGE_SCENE_SEGMENTATION/CELEBAMASK_HQ/CelebA-HQ-img/9195.jpg'
    source_label_path = '/media/baidu/DataRepo/IMAGE_SCENE_SEGMENTATION/CELEBAMASK_HQ/CelebAMaskHQ-mask/9195.png'

    source_image = tf.io.read_file(source_image_path)
    source_image = tf.image.decode_png(source_image)
    source_image.set_shape(shape=[1024, 1024, 3])
    source_image = tf.image.resize_bilinear(
        images=tf.expand_dims(source_image, axis=0),
        size=[512, 512]
    )
    source_image = tf.squeeze(source_image, axis=0)
    source_label_image = tf.io.read_file(source_label_path)
    source_label_image = tf.image.decode_png(source_label_image, channels=0)
    source_label_image.set_shape(shape=[512, 512, 1])
