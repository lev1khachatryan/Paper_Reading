...
class BiseNetV2CelebamaskhqMultiTrainer(object):
    """
    ...
    def __init__(self):
        ...
        self._celebamaskhq_io = celebamask_hq_tf_io.CelebamaskhqTfIO()
        self._train_dataset = self._celebamaskhq_io.train_dataset_reader
        self._val_dataset = self._celebamaskhq_io.val_dataset_reader
        ...
        LOG.info('Initialize celebamaskhq bisenetv2 multi gpu trainner complete')
            ...
            if epoch % self._snapshot_epoch == 0:
                if self._enable_miou:
                    if len(best_model) < 10:
                        ...
                        snapshot_model_name = 'celebamaskhq_val_miou={:.4f}.ckpt'.format(val_epoch_mious)
                    ...
                    snapshot_model_name = 'celebamaskhq_val_loss={:.4f}.ckpt'.format(val_epoch_losses)
                    snapshot_model_path = ops.join(self._model_save_dir, snapshot_model_name)
                    os.makedirs(self._model_save_dir, exist_ok=True)
                    self._saver.save(self._sess, snapshot_model_path, global_step=epoch)
...
if __name__ == '__main__':
    ...
    test code
    """
    worker = BiseNetV2CelebamaskhqMultiTrainer()
    print('Init complete')
